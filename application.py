...
def print_board(single_player, board):
...
def user_place_ships(board, ships):
    ...
    for ship in ships.keys():
        ...
        while not valid:
            ...
            row, col = get_coor()
            ori = v_or_h()
            valid = validate(board, ships[ship], row, col, ori)
            if not valid:
                print "Cannot place a ship there.\nPlease take a look at the board and try again."
                raw_input("Hit ENTER to continue")

        #place the ship
        board = place_ship(board, ships[ship], ship[0], ori, row, col)
        print_board("u", board)
...
def place_ship(board, ship, single_player, ori, row, col):
...
def validate(board, ship, row, col, ori):
...
def user_move(board):
    ...
    while True:
        row, col = get_coor()
        res = make_move(board, row, col)
        if res == "hit":
            print "Hit at " + str(row+1) + "," + str(col+1)
            check_sink(board, row, col)
            board[row][col] = '$'
            if check_win(board):
                return "WIN"
        elif res == "miss":
